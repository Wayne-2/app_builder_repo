  ...
  Future<void> _createAccount() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        final userModel = UserModel.fromMap(insertResponse);


      final monnify = MonnifyService();
      await monnify.createVirtualAccount(context, user.id);

        // Store in Riverpod
        if (mounted) {
          // if using ConsumerWidget context
          final container = ProviderScope.containerOf(context);
          container.read(userProfileProvider.notifier).setUser(userModel);
        }
        ...
        Future.delayed(const Duration(milliseconds: 700), () {
          Navigator.pushReplacement(
            context, MaterialPageRoute(builder: (context) => AppNavigator()),
            ...
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Image(width: 43, image: AssetImage(AppData.logo)),
                ),
                SizedBox(height: 5),
                Text(
                  "Create account",
